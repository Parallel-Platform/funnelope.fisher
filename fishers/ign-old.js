...
var categorizeContent = function (contentList) {
    ...
    if (contentList && contentList.length > 0) {
        ...
        _.each(contentList, function (content) {
             ...
             * (B) - Check theGameDB.net store - avoid as well
            ...
            _.each(combinedWordSets, function (wordSet) {
                ...
                if ((wordSet[0] === wordSet[0].toUpperCase()) && wordSet.length >= 4) {
                    ...
                    .then(function (snapshot) {
                        ...
                        if (result) {
                            //Break down the title into variosu words and peform a word frequency ranking against the title of the article.
                            //Which ever has the highest frequency rank - save that as the game associated with the article - save the id or title
                            var currGameMatches = _.map(result, function (game, key) { return { id: key, record: game }; });
                            var currRankedGames = [];
                            var currContentTitle = content.title;
                            var currWordTag = wordSet.trim();
                            
                            _.each(currGameMatches, function (currGame, currGameIndex) {
                                //break down game title into multiple words
                                var currTitle = latinizer.latinize(removeSpecialChars(currGame.record.title));
                                ...
                                var rankedGameAlready = _.findWhere(currRankedGames, { game: currGame.record.title });
                                if (!rankedGameAlready) {
                                    currRankedGames.push({ game: currGame.record.title, rank: currWordRank });
                                }
                            });
                            
                            //Get the game with the max frequency rank
                        ...
                        //check giantbomb api (last resort)
                        }
                    });
                }
            });
