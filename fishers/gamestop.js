...
//Article Store
var gamestopArticles = require('../content/gamestop.json');
gamestopArticles.gamestop = gamestopArticles.gamestop == null || gamestopArticles.gamestop == undefined ? [] : gamestopArticles.gamestop;
                    ...
                    if (searchResults !== null && searchResults !== undefined && searchResults.length > 0) {
                        ...
                        if (currRankedGames.length > 0) {
                            ...
                            if (maxRankedGame !== null && maxRankedGame !== undefined) {
                                ...
                                var articleResults = _.reduce(gamestopArticles.gamestop, function (memo, article) {
                                ...
                                if (articleResults !== null && articleResults !== undefined && articleResults.length > 0) {
                                    ...
                                    var articleJSON = JSON.stringify(gamestopArticles, null, 4);
                                    fs.writeFileSync("content/gamestop.json", articleJSON);
                                    
                                    console.log('updated gamestop content: ' + currContentTitle);
                                }
                                else {
                                    
                                    var itemCategory = '';
                                    
                                    //Determine gamestop category
                                    switch (content.feedtitle.trim()) {
                                        case 'GameStop Price Drops - All Platforms':
                                            itemCategory = 'pricedrop';
                                            break;

                                        case 'GameStop Best Sellers - All Platforms':
                                            itemCategory = 'bestsellers';
                                            break;

                                        case 'GameStop Coming Soon - All Platforms':
                                            itemCategory = 'comingsoon';
                                            break;

                                        case 'GameStop Now Shipping - All Platforms':
                                            itemCategory = 'newrelease';
                                            break;
                                    ...
                                    //Store the article
                                    gamestopArticles.gamestop.push(newContent);
                                    
                                    //write our new article to the json file.
                                    var articleJSON = JSON.stringify(gamestopArticles, null, 4);
                                    fs.writeFileSync("content/gamestop.json", articleJSON);
                                    
                                    console.log('saved gamestop content: ' + currContentTitle);
                                }
                            }
                        }
                    }
...
var pullCatch = function (feed) {
    console.log('DING: Caught some Gamestop fish...');
    ...
    parseXML(feed, options, function (err, result) {
        if (!err && result && result.rss && result.rss.channel && result.rss.channel.length > 0) {
            ...
            var category = 'news';
            var feedTitle = result.rss.channel[0].title !== null && result.rss.channel[0].title !== undefined && result.rss.channel[0].title.length > 0 ? result.rss.channel[0].title[0] : '';
            var content = [];
            ...
            _.each(result.rss.channel[0].item, function (feedItem) {
                ...
                content.push({
                    feedtitle: feedTitle !== null && feedTitle !== undefined && feedTitle !== '' ? feedTitle : '',
                    title: title, 
...
var gamestop_fisher = {
    goFishing: function () {
        console.log('DING: Fishing for Gamestop fish...');
        
        var gamestop_parent = this;
        var gamestop_source = _.findWhere(funneldata.sources, { name : "gamestop" });
        
        if (gamestop_source) {
            _.each(gamestop_source.feeds, function (feedSource) {
...
module.exports = gamestop_fisher;
